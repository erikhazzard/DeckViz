  ...
  DECKVIZ.Deck.create = function(deck) {
    ...
    return $.ajax({
      ...
      success: function(res) {
        var card, cardTypes, i, _i, _j, _len, _ref, _ref1;
        _ref = res.cards;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          card = _ref[_i];
          if (deck[card.name] > 0) {
            for (i = _j = 0, _ref1 = deck[card.name] - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
              finalDeck.push(card);
            }
            deck[card.name] = -1;
          }
  ...
  DECKVIZ.Deck.drawManaCurve = function(deck, originalDeck) {
    ...
    for (_i = 0, _len = deck.length; _i < _len; _i++) {
      ...
      if (curManaCost) {
        ...
        tmpCost = '';
        if (colorlessCost > 0) {
          for (i = _j = 0, _ref = colorlessCost - 1; 0 <= _ref ? _j <= _ref : _j >= _ref; i = 0 <= _ref ? ++_j : --_j) {
            tmpCost += 'C';
          }
        }
    ...
    svgEl.selectAll("text.label").data((function() {
      ...
      _results = [];
      for (num = _k = 0, _ref1 = maxManaCost - 1; 0 <= _ref1 ? _k <= _ref1 : _k >= _ref1; num = 0 <= _ref1 ? ++_k : --_k) {
        _results.push(num);
      }
      return _results;
